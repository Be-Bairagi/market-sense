...
class TechnicalIndicatorService:
    ...
    def compute_all(df: pd.DataFrame) -> Dict[str, float]:
        ...
        if df.empty or len(df) < 14:
            logger.warning(
                "Insufficient data for indicator computation (%d rows)", len(df)
            )
            return {}
        ...
        features["macd_diff_ratio"] = (
            (macd_ind.macd().iloc[-1] - macd_ind.macd_signal().iloc[-1])
            / close.iloc[-1]
            * 100
        )
        ...
        for period in EMA_WINDOWS:
            if len(df) >= period:
                ema = (
                    ta.trend.EMAIndicator(close, window=period)
                    .ema_indicator()
                    .iloc[-1]
                )
                features[f"price_vs_ema_{period}"] = (close.iloc[-1] - ema) / ema * 100
            else:
                features[f"price_vs_ema_{period}"] = 0.0
        ...
        for win in VOLATILITY_WINDOWS:
            features[f"volatility_{win}d"] = (
                log_returns.rolling(win).std().iloc[-1] * 100
            )

        # --- Volume ---
        vol_20d_avg = volume.rolling(20).mean().iloc[-1]
        features["volume_spike_ratio"] = (
            float(volume.iloc[-1]) / vol_20d_avg if vol_20d_avg > 0 else 1.0
        )

        # --- Candle Patterns (Standardized) ---
        features["body_size"] = abs(close.iloc[-1] - open_price.iloc[-1]) / open_price.iloc[-1]
        features["upper_shadow"] = (high.iloc[-1] - max(open_price.iloc[-1], close.iloc[-1])) / open_price.iloc[-1]
        features["lower_shadow"] = (min(open_price.iloc[-1], close.iloc[-1]) - low.iloc[-1]) / open_price.iloc[-1]
        
        if len(df) >= 2:
            features["overnight_gap"] = (open_price.iloc[-1] - close.iloc[-2]) / close.iloc[-2]
        else:
        ...
        for lag in LAG_PERIODS:
            if len(df) >= lag + 1:
                features[f"log_return_lag_{lag}"] = np.log(close.iloc[-1] / close.shift(lag).iloc[-1]) * 100
            else:
                features[f"log_return_lag_{lag}"] = 0.0

        # --- Price Position (Stationary relative to 52-week) ---
        high_52w = high.rolling(252, min_periods=1).max().iloc[-1]
        low_52w = low.rolling(252, min_periods=1).min().iloc[-1]
        features["dist_52w_high"] = (close.iloc[-1] - high_52w) / high_52w * 100
        features["dist_52w_low"] = (close.iloc[-1] - low_52w) / low_52w * 100
    ...
    def compute_all_history(df: pd.DataFrame) -> pd.DataFrame:
        ...
        log_returns = np.log(close / close.shift(1))
        for win in VOLATILITY_WINDOWS:
            hist_df[f"volatility_{win}d"] = log_returns.rolling(win).std() * 100

        # 4. Volume
        vol_avg = volume.rolling(20).mean()
        hist_df["volume_spike_ratio"] = volume / vol_avg.replace(0, 1)

        # 5. Shadows & Body (Standardized)
        hist_df["body_size"] = abs(close - open_price) / open_price
        hist_df["upper_shadow"] = (high - df[["Open", "Close"]].max(axis=1)) / open_price
        hist_df["lower_shadow"] = (df[["Open", "Close"]].min(axis=1) - low) / open_price
        hist_df["overnight_gap"] = (open_price - close.shift(1)) / close.shift(1)

        # 6. Lagged Log-Returns
        for lag in LAG_PERIODS:
            hist_df[f"log_return_lag_{lag}"] = np.log(close / close.shift(lag)) * 100
